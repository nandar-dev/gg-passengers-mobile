...
class RideReviewScreen extends StatefulWidget {
  const RideReviewScreen({super.key, this.bookingId});
...
class _RideReviewScreenState extends State<RideReviewScreen> {
  ...
  Future<void> _submitReview() async {
    if (_overallRating == 0) {
      AppMessage.info(context, 'Please rate your overall experience.');
      return;
    }

    final bookingId = widget.bookingId?.trim() ?? '';
    if (bookingId.isEmpty) {
      AppMessage.error(context, 'Booking id is missing for review submission.');
      return;
    }

    setState(() => _isSubmitting = true);

    final comment = _feedbackController.text.trim().isEmpty
        ? (_selectedQuickFeedback.isEmpty
              ? 'Great ride experience.'
              : _selectedQuickFeedback.join(', '))
        : _feedbackController.text.trim();

    try {
      await getIt<BookingRepository>().submitBookingReview(
        bookingId: bookingId,
      ...
      if (!mounted) return;
      AppMessage.error(context, 'Unable to submit review right now.');
    } finally {
      if (mounted) {
